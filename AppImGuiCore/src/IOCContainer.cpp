...
public:
    ...
    {
        m_factories[typeid(TInterface).name()] = std::make_shared<CFactory<TInterface>>([=]{return functor(GetObject<TS>()...);});
    }
    ...
    {
        m_factories[typeid(TInterface).name()] = std::make_shared<CFactory<TInterface>>([=]{return t;});
    }
