...
#include "imgui_stdlib.h" //for Input(std::string)
...
/*Texture LoadTextureFromFile(const std::string& filename){
	Texture tex;
	AppSystem::Image Img(filename);
	// Img.LoadTextureFromFile(filename.c_str());
	tex.id = (ImTextureID)Img.GetDescriptorSet();
	tex.h = Img.GetHeight();
	tex.w = Img.GetWidth();
	return tex;
}*/
#endif
inline Texture LoadTextureFromFileImRad(const std::string& filename)
	{
		Texture tex;
		AppSystem::Image Img(filename);
		// Img.LoadTextureFromFile(filename.c_str());
