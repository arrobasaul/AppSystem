...
void Image::DoExport(std::ostream& os, UIContext& ctx)
{
	...
	ctx.ind_up();
	os << ctx.ind << fieldName.to_arg() << " = ImRad::LoadTextureFromFileImRad(" << fileName.to_arg() << ");\n";
	ctx.ind_down();
...
void Image::DoImport(const cpp::stmt_iterator& sit, UIContext& ctx)
{
	...
	{
		auto i = sit->line.find("ImRad::LoadTextureFromFileImRad(");
		if (i != std::string::npos)
...
void Image::RefreshTexture(UIContext& ctx)
{
	...
	tex = ImRad::LoadTextureFromFileImRad(fname);
	if (!tex) 
	{
		if (!ctx.importState) {
